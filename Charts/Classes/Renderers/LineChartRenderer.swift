...
//            drawLinearFill(context: context, dataSet: dataSet, minx: minx, maxx: maxx, trans: trans)
    ...
    private func generatePath(dataSet dataSet: ILineChartDataSet, fillMin: CGFloat, from: Int, to: Int,  matrix: CGAffineTransform) -> CGPath
    {
        var mat = matrix
        ...
        if e != nil
        {
            CGPathMoveToPoint(generatedPath, &mat, CGFloat(from), CGFloat(e.x) * CGFloat(phaseY))
        }
        // create a new path
        for (var x = from + 1, count = Int(ceil(CGFloat(to - from) * CGFloat(phaseX) + CGFloat(from))); x < count; x += 1)
        {
            guard let e = dataSet.entryForIndex(x) else { continue }
            CGPathAddLineToPoint(generatedPath, &mat, CGFloat(x), CGFloat(e.x) * CGFloat(phaseY))
        }
    ...
    internal func drawGradientLine(context context: CGContext, dataSet: ILineChartDataSet, spline: CGPath, matrix: CGAffineTransform)
    {
        ...
        //Set middle colors
        for (var i = 0; i < dataSet.gradientPositions!.count; i += 1) {
        ...
        for (var i = 0; i < dataSet.colors.count; i += 1) {
